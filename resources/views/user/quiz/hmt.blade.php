...
@section('content')
<div class="h-full py-4 flex absolute top-0 left-0 w-full"
    x-data="quiz({{ $questions->toJson() }}, {{ json_encode($settings[\App\Models\Setting::HMT_DESCRIPTION]) }}, {{ json_encode($settings[\App\Models\Setting::HMT_PRIVACY]) }})">
    ...
    <div class="max-w-4xl grow mx-auto px-6 lg:px-12">
        ...
        <!-- ===================== -->
        <!-- STAGE 2: DEMO QUESTION 1 -->
        <!-- ===================== -->
        <template x-if="stage === 1">
            <div class="shadow-xl p-6 rounded-xl border border-orange-200 bg-white space-y-5">
                <h2 class="text-xl font-semibold text-center text-orange-600">Contoh Soal 1</h2>
                <p class="text-center text-sm text-gray-600">Klik salah satu jawaban di bawah untuk mencoba interaksi.</p>

                <div class="flex justify-center mb-6">
                    <img src="{{ asset('images/demo/hmt-sample-question.png') }}" alt="Contoh Soal HMT"
                        class="rounded-lg shadow max-h-64">
                </div>

                <div class="grid md:grid-cols-6 grid-cols-3 gap-3" x-data="{ demoSelected1: null }">
                    <template x-for="i in 6">
                        <button type="button" @click="demoSelected1 = i"
                            :class="[
                                'p-2 rounded-lg transition flex items-center justify-center',
                                demoSelected1 === i ? 'bg-orange-200 ring-2 ring-orange-500'
                ...
                <div class="text-center pt-6">
                    <button @click="nextStage()" class="bg-orange-500 hover:bg-orange-600 text-white px-5 py-2 rounded transition">
                        <i class="fas fa-arrow-right"></i> Lanjut ke Contoh 2
                    </button>
                </div>
            </div>
        </template>
        ...
        <!-- ===================== -->
        <!-- STAGE 3: DEMO QUESTION 2 -->
        <!-- ===================== -->
        <template x-if="stage === 2">
            <div class="shadow-xl p-6 rounded-xl border border-orange-200 bg-white space-y-5">
                <h2 class="text-xl font-semibold text-center text-orange-600">Contoh Soal 2</h2>
                ...
                <div class="grid md:grid-cols-6 grid-cols-3 gap-3" x-data="{ demoSelected2: null }">
                    <template x-for="i in 6">
                        <button type="button" @click="demoSelected2 = i"
                            :class="[
                                'p-2 rounded-lg transition flex items-center justify-center',
        ...
        <!-- ======================= -->
        <template x-if="stage === 3 && currentQuestion">
        ...
        <!-- STAGE 5: SELESAI -->
        <!-- ======================= -->
        <div x-show="stage === 4" class="text-center mt-10">
...
function quiz(questions, hmtDesc, hmtPrivacy) {
    return {
        stage: 0, // 0 = Preface, 1 = Demo1, 2 = Demo2, 3 = Quiz, 4 = Finished
        questions,
        description: hmtDesc,
        privacy: hmtPrivacy,
        privacyAgreed: false,
        ...
        async startQuiz() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    this.stage = 3; // Masuk ke soal asli
                    this.loadQuestion(0);
                } else {
