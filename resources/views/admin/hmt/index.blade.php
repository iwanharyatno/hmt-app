...
<div class="bg-white p-6 rounded-xl shadow-md">
    ...
    <table class="w-full text-sm">
      ...
      <tbody class="text-gray-700">
        ...
        <tr class="border-t hover:bg-orange-50" x-data="{ showConfirm: false }">
          ...
          <td class="py-3 px-4">
            @if($q->question_path)
              <img src="{{ Storage::url($q->question_path) }}" alt="Soal" class="h-12 rounded border object-cover">
            @else
          ...
          <td class="py-3 px-4 space-x-2">
            ...
            <!-- Konfirmasi -->
            <div x-show="showConfirm" x-cloak class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40">
              <div class="bg-white p-6 rounded shadow max-w-sm w-full">
                <p class="text-gray-700 mb-4">Yakin ingin menghapus pertanyaan ini?</p>
                <div class="flex justify-end space-x-3">
                  <button @click="showConfirm = false" class="px-3 py-1 bg-gray-300 rounded hover:bg-gray-400">Batal</button>
  ...
  <div class="space-y-4 sm:hidden">
    ...
    <div class="border rounded-lg p-4 shadow hover:bg-orange-50" x-data="{ showConfirm: false }">
      ...
      @if($q->question_path)
        <img src="{{ Storage::url($q->question_path) }}" alt="Soal" class="h-20 rounded border object-cover">
      @endif
